...
SOURCE_LABELS = {
    ...
    "invoice": "invoice or tax invoice",
...
REVIEW_SUPPORT_NOTE = (
    ...
    "A human reviewer must make any final internal-control, audit, legal, or "
    "accounting conclusion."
)


def normalize_case(payload):
    """Support both the original flat demo input and the structured case format."""
    ...
    return {
        ...
        "records": {
            ...
            if key not in {"case_id", "control_objective", "required_evidence"}
        },
    }
...
def review_note(status):
    notes = {
        ...
        "missing_evidence": "Required evidence is missing. Request the missing document or document why it is not required before making a control conclusion.",
        ...
        "needs_follow_up": "The provided evidence needs reviewer follow-up before a control conclusion can be made.",
...
def triage_evidence(records):
    case = normalize_case(records)
    ...
    for item in expected:
        if item not in records or not is_present(records[item]):
            add_finding(
                ...
                f"Request the {item} or document why it is not required for this control.",
    ...
    if len(set(counterparty_sources.values())) > 1:
        add_finding(
            ...
            "Confirm whether the vendor names refer to the same legal entity.",
            evidence=[
                {
                    "source": SOURCE_LABELS.get(document, document),
                    "document": document,
